...
fn main() {
    ...
    while input_raw != gui::EXIT_STR {
        ...
        if input_vec.len() > 1
        {
            ...
            {
                parse_input(input, &mut sample_size, &mut sample_values, &mut weight_values, &mut show_sample_values, 
                    &mut show_weight_values, &mut show_sample_size, &mut show_warnings);
            }
        ...
        parse_input(input_raw.as_str(), &mut sample_size, &mut sample_values, &mut weight_values, &mut show_sample_values, 
            &mut show_weight_values, &mut show_sample_size, &mut show_warnings);
    ...
    fn parse_input(
        ...
        mut sample_values : &mut Vec<f32>,
        mut weight_values : &mut Vec<f32>,
        show_sample_values: &mut bool,
        show_weight_values: &mut bool,
        show_sample_size  : &mut bool,
        show_warnings     : &mut bool,
    ) {
        match input {
            gui::SET_VALUES_STR => {
                println!("\n{} {} {}", "Type the".italic(), *sample_size, "new sample values:".italic());
                sample_values.clear();
                functions::change_vec_values(&mut sample_values, *sample_size);
            },
            gui::SET_WEIGHTS_STR => {
                println!("\n{} {} {}", "Type the".italic(), *sample_size, "new weight values:".italic());
                weight_values.clear();
            ...
            gui::SET_SAMPLE_SIZE_STR => {
                ...
                *sample_size = match new_input.parse::<usize>() {
                    Ok(v) => v,
                    Err(_) => 5
                };
            ...
            gui::SET_VALUE_AT_INDEX_STR => {
                functions::change_vec_value_at_index(&mut sample_values);
            },

            gui::SET_WEIGHT_AT_INDEX_STR => {
                functions::change_vec_value_at_index(&mut weight_values);
            },
            ...
            gui::RESET_VALUE_STR => functions::reset_vec_values(&mut sample_values, *sample_size),
            gui::RESET_WEIGHTS_STR => functions::reset_vec_values(&mut weight_values, *sample_size),
